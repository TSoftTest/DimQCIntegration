...
import ar.com.tssa.serena.connectors.alm.Entity.Fields;
...
import ar.com.tssa.serena.connectors.utils.ConnectorQc;
...
import ar.com.tssa.serena.connectors.utils.parser.ArmadoHtml;
import ar.com.tssa.serena.connectors.utils.xml.Audits.Audit;
...
public class QcTarea {
	...
	/**
	 * Metodo que hace un update de Dimensions a Qc de todos los atributos definidos en mapeo.xml y los obligatorios
	 * 
	 * @param requestDim
	 *           Request de Qc
	  @param requestQc
	...
	public void updateQcRequest(Entity requestQc, Request requestDim, DimensionsTarea dimTarea) throws Exception{
		...
		try{
			...
			/*
			 * Si el estado de Dimensions no es uno de los estados que se excluyen de ser replicados a Qc, se procede a su sincronizacion.
			 */
		...
		if (CampoMultiple.getInstance().esCampoConCombinacion(attributeDim)){
			...
			if (objAtributoDimensions instanceof java.util.Vector){
				...
				if (Mapeo.getInstance().isMemo(attributeName)){
					for (ListIterator<String> iterator = vectorAtributoDimensions.listIterator(); iterator
							.hasNext();) {
	...
	public void updateFechaSincronizacionQc (Entity requestQc) throws Exception{				
		...
		/*
		 * Vuelvo a buscar el objeto para asegurarme que tiene los valores actualizados.
		 */
		...
		try{
			...
			//Se actualiza la entidad para que se modifique la fecha de auditoria y sea lo mas parecida a la de sincronizacion
	...
	/**Verifica si la entidad (request) de QC esta desbloqueada,
	 *	es decir ningun usuario la esta utilizando.
	 * @param requestQc
	...
	public void pasajeDeEstado(Entity requestQc) throws Exception{
		...
		/*
		 * No se verifica si se trata de un Estado Excluido ya que el dato "se toma directamente de Qc" (se lee de
		 * la configuracion pero se supone que es el nombre que tiene en la aplicacion realmente).
		 * Esto no pasa en el pasaje de estado de Dimensions, ya que ahi se parte de Qc para ir a Dimensions y
		 * en ese caso se necesita que si o si este el estado.
		 */
		try{
			//Actualizo el estado
	...
	private void updateMultivalueAttribute(Entity requestQc, Request requestDim, String attributeName, DimensionsTarea dimTarea) throws UnsupportedEncodingException {
		 ...
		 * mientras que si en Qc no se encuentran repetidos en la lista a seleccionar se produce una excepcion.
		 */
	...
	/**
	 * Metodo que se retorna el contenido del campo que es una agrupacion (combinacion
	 * de campos) en Dimensions en un solo string.
	 * Solo sirve para el campo 'comentarios' y bajo la estructura que pidio el cliente,
	 * para el resto de los campos devuelve una cadena vacia.
	...
	private String obtenerContenidoCampoMultiple(String attributeName, String attributeDim, Request requestDim, DimensionsTarea dimTarea, String productoDim) throws Exception{
		...
		//Limpio contenido del campo auxiliar de carga de los comentarios en Dimensions
	...
	/**
	 * Metodo auxiliar para terminar de obtener el contenido del campo que es una agrupacion (combinacion
	 * de campos) en Dimensions en un solo string.
	 * Solo se utiliza para el campo 'comentarios'.
	 */
	private String obtenerComentarioAuxiliarMultiple(List<String> atributosDim, Request requestDim, DimensionsTarea dimTarea){
		...
		//Esta parte se hace porque en Dimensions se pueden agregar campos arriba o debajo del actual.
		String contenidoAux = "";
	...
	/**
	 * Metodo retorna el contenido de cada campo que forma el bloque o el campo que es una agrupacion
	...
	/**
	 * Metodo que valida que los campos que forma el bloque o el campo que es una agrupacion
	 * (combinacion de campos) en Dimensions contenga la misma cantidad de registros. 
	 */
