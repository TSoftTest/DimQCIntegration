package com.tsoft.dimqc.connectors.core;
...
public class Sincronizador {
	...
	private synchronized void sincronizar(DimensionsTarea dimTarea, QcTarea qcTarea, List<Long> idsQc, String nombreEsquemaBaseQc) {
		...
		// Formato utilizado para parsear las fechas de sincronizacion y auditoria
		// de los request de QC y Dimensions
				...
				for (Long idQc : idsQc) {
					try {
						...
						if (dimensionsId != null && !dimensionsId.equals("")) {
							...
							// Si el Request que traje de Dimensions es null, es decir alguna
							// vez existio, esto no deberia pasar, loggeo el error
							if (requestDim != null) {
								logger.debug("Se encontro el request en Dimensions");
								logger.debug("Request de Dimensions: " + "Nombre: " + requestDim.getName() + " OBJECT_ID: " + requestDim.getAttribute(SystemAttributes.OBJECT_ID) + " TITTLE: "
								    + requestDim.getAttribute(SystemAttributes.TITLE));
								...
								// Si el request de Dimensions no es null, pregunto si es el del
								// tipo BUG
									...
									try {
										...
										try {
											...
											if (comparacionFechasQc) {
												// Valido que la ultima modificacion la haya realizado
												// un usuario distinto al de la sincronizacion
												/*
												 * Se agrega el dato sobre la base de Qc porque Qc no
												 * esta informando bien la auditoria para todos los
												 * bugs. Algunos tiran error al querer consultarse la
												 * misma; por lo tanto, en caso de error, se va a buscar
												 * el dato a la base.
												 */
											...
											try {
												...
												// Tiene fecha de sincronizacion, obtengo la de
												// auditoria
												...
												try {
													...
													if (comparacionFechasQc && comparacionFechasDim) { // Si
														...
														// Si solo la diferencia de QC es mayor o igual a X
														// minutos, copio de QC a Dimensions
														...
														// Si solo la diferencia de Dim es mayor o igual a X
														// minutos, copio de Dimensions a QC
														logger.debug("Se modifico el request de Dimensions.");
														copiarDimensionsAQc(requestQc, requestDim, logger, qcTarea, dimTarea);
													} else {
														// Ningun request tiene una diferencia mayor o igual
														// a X minutos. No cambiaron
														logger.debug("Los requests se encuentran sincronizados.");
													}

												} catch (ParseException e) {
													// La fecha de auditoria del request de Dimensions no
													// existe, se debe copiar segun prioridad
													logger.debug("El request de Dimensions no tiene fecha de auditoria.");
													copiarSegunPrioridad(requestQc, requestDim, logger, qcTarea, dimTarea, nombreEsquemaBaseQc, proyectosEnDim);
												}
											} catch (ParseException e) {
												// La fecha de sincronizacion del request de Dimensions
												// no existe, se debe copiar segun prioridad
												logger.debug("El request de Dimensions no tiene fecha de sincronizacion.");
												copiarSegunPrioridad(requestQc, requestDim, logger, qcTarea, dimTarea, nombreEsquemaBaseQc, proyectosEnDim);
											}
										} catch (ParseException e) {
											// La fecha de sincronizacion del request de QC no existe,
											// se debe copiar segun prioridad
										...
										// Significa que es la primera vez que se va a sincronizar
										// el elemento, se copia segun prioridad
							...
							if (!replica) {
								logger.debug("El request de QC no tiene un request en Dimensions asociado y en estado " + attributeFieldEstado.getValue().get(0).getValue()
								...
								if (productoDim != null) {
									/*
									 * Solo creo el request en Dimensions si la entidad (request)
									 * en QC esta desbloqueada es decir ningun usuario la esta
									 * utilizando, caso contrario el request se replica en la
									 * proxima iteracion
									 */
									...
									if (!locked) {
										...
										if (requestId != null) {
											...
											// Con esto se espera que no se creen Request de mas y a
											// su vez se hace porque solo puedo
											// acceder a la configuracion de los atributos de
											// Dimensions mediante un Request
											boolean completoCarga = dimTarea.completarCreacionRequestEnDim(idDimField.getValue().get(0).getValue(), requestQc, productoDim, qcTarea, nombreEsquemaBaseQc,
											    proyectosEnDim);
	...
	/**
	 * Metodo que verifica si se deben o no sincronizar los archivos de QC a
	 * Dimensions y los sincroniza
	 * 
	 * @param requestQc
	 * @param requestId
	 */
	private void filesSynchronizationQCToDim(Entity requestQc, Request requestDim, Logger logger, QcTarea qcTarea, DimensionsTarea dimTarea) {
		...
		try {
			...
			if ("Y".equals(attachment)) {
				...
				// Obtengo los attachments que ya tiene el request y verifico si ya
				// existen
	...
	/**
	 * Metodo que copia un request a otro segun la prioridad definida en el
	 * archivo properties
	 * 
	 * @param requestQc
	 * @param requestDim
	 * @throws Exception
	 */
	private void copiarSegunPrioridad(Entity requestQc, Request requestDim, Logger logger, QcTarea qcTarea, DimensionsTarea dimTarea, String nombreEsquemaBaseQc,
	    List<String> proyectosEnDim) throws Exception {
	 ...
	 * @throws Exception
	 */
	private void copiarQcADimensions(Entity requestQc, Request requestDim, Logger logger, QcTarea qcTarea, DimensionsTarea dimTarea, String nombreEsquemaBaseQc,
	    List<String> proyectosEnDim) {
		try {
	...
	/**
	 * Compara dos fechas y me dice si su diferencia es mayor a la establecida en
	 * las propiedades
	 * 
	 * @param fechaSinc
	 * @param fechaAud
	...
	private boolean compareDates(Date fechaSinc, Date fechaAud, Logger logger) {
		...
		// Si la diferencia es de X o mas minutos sincronizo el request, caso
		// contrario considero que el request esta sincronizado
		return diffMinutes >= ConnectorProperties.getInstance().getDifTime();
	}

	/**
	 * Metodo que elimina la carpeta temporan utilizada para la descarga de
	 * archivos durante la sincronzacion de los mismos
	 */
