...
public class ArmadoHtml {
		...
		if(w instanceof HtmlWrapperComposite){
			...
			for (TipoTag ele : comp.getTipoTagHijos()){
				for (int i = 0; i < cantHijos; i++){
					comp.addHijo(wrap(ele,1)); //puse 1 xq tal cual esta armado solo tiene un hijo cada tag
				}
			}
		}
	...
	public Map<Integer,Tag> obtenerTags(String texto){
		...
		while (scanner.hasNextLine() || !leer) {
	    	...
	    	if (TipoTag.DIV.getValue().equals(tipoTag.getValue())){
	    		...
	    		leer = true;
	    	} else if (TipoTag.OL.getValue().equals(tipoTag.getValue()) || TipoTag.UL.getValue().equals(tipoTag.getValue())){ //este y el siguiente pueden ir juntos
		...
		//Por defecto los ordena de menor a mayor. En este caso, se van a ordenar por numero de linea de menor a mayor.
	...
	public static boolean isListaNumerica (String linea) {
		...
		String regex = "^ {" + HtmlWrapperComposite.SANGRIA_SIMBOLO.length() + "}[1-9]+\\" + HtmlWrapperComposite.SIMBOLO_OL + " {" + HtmlWrapperComposite.SEGUNDA_SANGRIA_SIMBOLO.length() + "}.*";
