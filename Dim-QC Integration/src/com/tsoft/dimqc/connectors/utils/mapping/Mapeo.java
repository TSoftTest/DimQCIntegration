...
public class Mapeo {
				...
				for (int k = 0; k < elemento.getElementsByTagName(ATTRIBUTE_MAPPING).getLength(); k++) {
					...
					for (int j = 0; j < listaAtributosMapping.getLength(); j++) {
						...
						if (ATTRIBUTE_NAME.equals(uno.getNodeName())) {
							...
							if (uno.getNodeName() != null && ("false".equals(uno.getNodeValue().toLowerCase()) || "true".equals(uno.getNodeValue().toLowerCase()))) {
	...
	public String getAttributeByName(String reqType, String name) throws PropertiesException {
		Pair<String, String> par = atributos.get(name); // Me devuelve el par de
																										// values para un atributo
																										// en particular
	...
	private void cargarAtributosGenerales() {
		...
		if (atributos != null) {
			...
			// Se remueven los que no son necesarios sincronizar o se sincronizan
			// obligatoriamente
