...
import ar.com.tssa.serena.connectors.alm.Entity.Fields.Field.Value;
import ar.com.tssa.serena.connectors.alm.QcTarea;
import ar.com.tssa.serena.connectors.exceptions.ServiceException;
import ar.com.tssa.serena.connectors.utils.ConnectorProperties;
import ar.com.tssa.serena.connectors.utils.mapping.CampoMultiple;
import ar.com.tssa.serena.connectors.utils.mapping.Estado;
import ar.com.tssa.serena.connectors.utils.mapping.EstadoExcluido;
import ar.com.tssa.serena.connectors.utils.mapping.Mapeo;
import ar.com.tssa.serena.connectors.utils.mapping.Pair;
import ar.com.tssa.serena.connectors.utils.parser.HtmlElement;
import ar.com.tssa.serena.connectors.utils.parser.HtmlParser;
import ar.com.tssa.serena.connectors.utils.xml.FieldsQc;
import ar.com.tssa.serena.database.ConsultasSql;
import ar.com.tssa.serena.database.TipoConexion;
...
public class DimensionsTarea {
	...
	/**
	 * Metodo que hace un update de Qc a Dimensions de todos los atributos definidos en  mapeo.xml
	 * 
	 * @param requestDim
	...
	public boolean updateDimensionsRequest(Request requestDim, Entity requestQc, QcTarea qcTarea, String nombreEsquemaQC, List<String> proyectosEnDim) throws Exception{
		...
		try{
			...
			//Actualizo la descripcionGeneral (se encuentra en formato html) - Campo de la solapa: General
			updateDescriptionAttribute(requestDim, requestQc, "descripcionGeneral");
			
			//Actualizo la descripcion (se encuentra en formato html) - Campo de la solapa: Attributes
			...
			/*
			 * Si el estado de Qc no es uno de los estados que se excluyen de ser replicados a Dimension, se procede a su sincronizacion 
			 * y de ser necesario se hace tambien el pasaje de estado.
			 */
			if (qcStatus != null && !"".equals(qcStatus) && !EstadoExcluido.getInstance().isEstadoExcluidoQc(qcStatus)){
				...
				try{
					...
					if(!"".equals(dimStatus)){ //Si esta vacio significa que no existe su equivalente
					...
					logger.debug(e); //Puede tirar exepcion de que el request ya se encuentra en ese estado
				}
	...
	/** Realiza el update del campo descripcionGeneral dado que se encuentra en formato HTML y necesita un tratado especial 
	 * @param requestDim
	 * @param requestQc
	 * @throws Exception 
	 */
	...
	/** Realiza el update del campo comentario dado que se encuentra en formato HTML y necesita un tratado especial 
	 * @param requestDim
	 * @param requestQc
	 * @throws Exception 
	 */
	...
	/** Realiza el update del campo tipo tree (Ej: Detectado en release) dado que se obtiene del attributo del valor y no del valor,  y necesita un tratado especial 
	 * @param requestDim
	 * @param requestQc
	 * @throws Exception 
	 */
	...
	/** Realiza el update de los campos especificados en el mapeo.xml, salvo aquellos que se copian obligatoriamente
	 * @param requestDim
	 * @param requestQc
	 * @param attributeName
	 * @throws Exception 
	 */
	private void updateGeneralAttribute(Request requestDim,Entity requestQc,String attributeName, String nombreEsquemaQC, Boolean llamadaRecursiva, List<String> proyectosEnDim) throws Exception{
	...
	public void cargarArchivos (Request requestDim, String path, String nombreBajada) throws Exception{		
		...
		try{
			...
			RequestAttachmentDetails[] archivos = new RequestAttachmentDetails[]{
					archivo
			};
	 ...
	 */
	private void updateMultivalueAttribute(Request requestDim,Entity requestQc,String attributeName, String nombreEsquemaQC, Boolean llamadaRecursiva, List<String> proyectosEnDim) throws UnsupportedEncodingException{
		String attributeDim = null;
		String attributeQc = null;
		logger.debug("Setteando el atributo '" + attributeName + "' de Qc a Dimensions.");
		attributeDim= Mapeo.getInstance().getAttributeByName(Mapeo.DIM, attributeName);
	...
	/**
	 * Metodo que modifica la cantidad maxima de caracteres permitida que tiene
	 * un campo.
	 */
	...
	/**
	 * Metodo que actualiza la lista de valores permitidos de
	 * un campo.
	 * @param nombreAtributo : El nombre del atributo de dimensions cuyo Validset se desea actualizar
	 * @param idDeLista : El ID de la lista en QualityCenter para obtener los valores
	 */
	@SuppressWarnings("unchecked")
	public boolean actualizarValidSet(String nombreAtributo, int idDeLista, String nombreEsquemaQC, List<String> proyectosEnDim){
		...
		try {
			...
			if (TipoConexion.ORACLE.equals(ConnectorProperties.getInstance().getQcDataBaseTipo())){
					...
					listaDeValoresQC = ConsultasSql.obtenerListaDeValoresParaCampoDesdeQC(String.valueOf(idDeLista), ConnectorProperties.getInstance().getQcDataBaseTipo(), 
								ConnectorProperties.getInstance().getQcDataBaseSDI(), nombreEsquemaQC, proyectosEnDim);
				
			} else if (TipoConexion.SQL_SERVER.equals(ConnectorProperties.getInstance().getQcDataBaseTipo())){			
	
					listaDeValoresQC = ConsultasSql.obtenerListaDeValoresParaCampoDesdeQC(String.valueOf(idDeLista), ConnectorProperties.getInstance().getQcDataBaseTipo(), nombreEsquemaQC, "", proyectosEnDim);
			...
			if (!listaDeValoresQC.isEmpty()){
				...
				for (ListIterator<String> iterator = listaDeValoresQC.listIterator(); iterator
						.hasNext();) {
	...
	/**
	 * Metodo que modifica el tipo de campo que es a multiple en caso de que no lo sea.
	 */
	...
	/**
	 * Metodo que deja los campos que se utilizaron para cargar el campo Comentario
	 * sin contenido.
	 * Para hacerlo cambia la configuracion del campo agrupador de ser necesario para
	 * realizar el cambio y luego vuelve a dejarlo con la configuracion que tenia previamente.
	 * @throws Exception 
	 */
	public void limpiarComentarioAuxiliar(List<String> atributosDim, Request requestDim, String nombreAgrupador, String productoDim) throws Exception{
		...
		if (!ajustoCantidadCampos){
			...
			/*
			 * Solo funciona en una configuracion (Block Update Type) de las dos posibles:
			 * 		1) Allow users to add, modify and delete rows.    (CON ESTA CONFIGURACION SI ANDA)
			 * 		2) Do not allow user to modify or delete rows; only to append new rows.    (CON ESTA CONFIGURACION NO FUNCIONA)
			 * En la base las mismas son representadas:
			 * 		1) Allow users to add, modify and delete rows. ->  ATTR_1 = 1
			 * 		2) Do not allow user to modify or delete rows; only to append new rows. ->  ATTR_1 = 2
			 */
	...
	/**
	 * Metodo que determina si un campo contiene la cantidad de caracteres maxima necesaria
	 * para cargar el contenido que se especifica.
	 */
	public boolean validaCantidadCaracteresCampo(String attributeName, String contenido) {
		...
		if (attributeDefinition == null){
			//logger.debug("No se pudo obtener la configuración del campo '" + attributeName + "' de Dimensions");
			return false;
		}
		...
		try {
			...
			//logger.error("Se produjo un error al obtener la cantidad máxima de caracteres que soporta actualmente el campo '" + attributeName + "' de Dimensions; se actualizará a la máxima.");
			return false;
		}
	...
	/**
	 * En caso de que el contenido sea mayor a la cantidad maxima de caracteres que
	 * permite ese campo, se modifica esa cantidad al maximo general.
	 * Si igualmente el contenido es mayor, se trunca el contenido para poder cargar el
	 * campo sin problemas.
	 * @throws Exception 
	 */
	 ...
	 * Si igualmente el contenido es mayor, se trunca el contenido para poder cargar el
	 * campo sin problemas.
	 * Este metodo se utiliza solo para el atributo Descripcion (tanto el de la solapa general
	 * como atributos). Esto se hace para lograr que ambos se vean igual.
	 * @throws Exception 
	 * 
	 */
	public String ajustarContenidoACantidadCaracteresDescripcion(String attributeName, String contenido) throws Exception{
		...
		//Esto es para ajustar la cantidad de caracteres que se pueden guardar en la base.
	...
	/**
	 * Metodo que se utiliza para hacer el paraje de Estado de New a Open en Dimensions.
	 * @throws UnsupportedEncodingException 
	 * 
	 */
	private void pasajeDeEstadoEnDimensions(Entity requestQc, Request request) throws UnsupportedEncodingException{
		...
		try{
			...
			if (ConnectorProperties.getInstance().isEstadoInicialQc(qcStatus)){
				...
				// Se verifica que el estado no sea uno de los "Estados Excluidos" de ser sincronizados.
				if (estadoFinalQc != null && !EstadoExcluido.getInstance().isEstadoExcluidoQc(estadoFinalQc)){
					...
					if(estadoFinalDim != null && !"".equals(estadoFinalDim)){ //Si esta vacio significa que no existe su equivalente												
			...
			logger.debug(e);//Puede tirar exepcion de que el request ya se encuentra en ese estado, o que el estado que se quiere settear no existe
		}	
	}
	...
	public String descargarArchivo(RequestAttachment dimAttachment) {
		...
		try {
			...
			String fileFolder = ConnectorProperties.getInstance().getDimensionsDownloadFolder(); //Carpeta donde esta corriendo el servicio (depende de windows)
			String dimensionsDownloadFileName = ConnectorProperties.getInstance().getDimensionsDownloadFileName(); //nombre con el que se va a crear el archivo 
			String tempFolder = ConnectorProperties.getInstance().getRutaArchivosTemporales(); //Carpeta a donde se  va a copiar el archivo
			String attachmentName = dimAttachment.getName(); //nombre del attachment, con el que se va a crear el archivo definitivo
	...
	public void copyFiles(String source, String dest) throws Exception {
		...
		try {
			...
			try { if (outputChannel != null) inputChannel.close(); } catch (Exception e) {}
			try { if (fileInputStream != null) fileInputStream.close(); } catch (Exception e) {} 
			try { if (fileOutputStream != null) fileOutputStream.close(); fileOutputStream.close();} catch (Exception e) {}			
		}
